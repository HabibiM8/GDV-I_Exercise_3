...
{
    // Retrieve Projection and ModelView Matrices from RenderState
    ...
    Plane planes[6];
    const float *m = clipMatrix.data(); // OpenGL stores matrices in column-major order
    ...
    for (const auto &plane : planes)
    {
        ...
        // If all corners are outside, cull the object
        if (allOutside)
        {
            return false;
        }
